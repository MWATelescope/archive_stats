...
parser = argparse.ArgumentParser(description="Plot archive usage broken down by science theme.")
parser.add_argument("infile", type=str, help="CSV file with storage data per project ID.")
parser.add_argument(
    "--outdir", type=str, default=None, help="Output directory for plots. Default is current directory."
)
parser.add_argument("--print_stats", help="Print some stats to the screen.", action="store_true")
parser.add_argument("--ext", default="png", type=str, help="Plot file extension. Default is png.")
...
cat_lookup_align = ["EoR\t", "SHI\t", "GEG\t", "Transients", "Pulsars and FT", "Calibration", "Misc\t"]
cats = {
    "Sun Drifts": 1,
    "GLEAM": 2,
    "FRBs with the MWA": 4,
...
if args.print_stats:
    ...
    print(f"Volume fraction captured by my incomplete categorization: {np.sum(cat_totals) / np.sum(vols)}")
    print("\n")
...
plt.tight_layout()
outfile = os.path.join(args.outdir, f"cumulative_frac.{args.ext}")
plt.savefig(outfile)
...
plt.tight_layout()
outfile = os.path.join(args.outdir, f"volume_per_swg.{args.ext}")
plt.savefig(outfile)
